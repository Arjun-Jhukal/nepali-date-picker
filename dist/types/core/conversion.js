import { BS_MIN, BS_MAX, BS_AD_ANCHOR, getDaysInMonth, getDaysInYear, } from './data-loader';
...
const ANCHOR_OFFSET = daysFromEra(BS_AD_ANCHOR.bs.year, BS_AD_ANCHOR.bs.month, BS_AD_ANCHOR.bs.day);
// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------
/**
...
export function adToBs(date) {
    if (!(date instanceof Date)) {
        throw new TypeError(`Expected Date object, got ${date === null ? 'null' : typeof date}`);
    }
    ...
    if (remaining > 0) {
        while (remaining > 0) {
            ...
            if (remaining <= daysLeft) {
                ...
                if (year > BS_MAX) {
                    throw new RangeError(`AD date ${date.toDateString()} exceeds supported BS range (max ${BS_MAX})`);
                }
            }
        }
    }
        ...
        while (remaining < 0) {
            if (day + remaining >= 1) {
                ...
                if (year < BS_MIN) {
                    throw new RangeError(`AD date ${date.toDateString()} predates supported BS range (min ${BS_MIN})`);
                }
