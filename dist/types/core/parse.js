...
export function parseDate(str, format) {
    ...
    if (!match || !match.groups) {
        throw new Error(`Cannot parse '${str}' as date with format '${format}'`);
    }
    const year = parseInt((_a = match.groups['year']) !== null && _a !== void 0 ? _a : '', 10);
    const month = parseInt((_b = match.groups['month']) !== null && _b !== void 0 ? _b : '', 10);
    const day = parseInt((_c = match.groups['day']) !== null && _c !== void 0 ? _c : '', 10);
    ...
    if (!isValidBSDate(year, month, day)) {
        throw new RangeError(`BS day ${day} is invalid for ${year}/${month}`);
    }
