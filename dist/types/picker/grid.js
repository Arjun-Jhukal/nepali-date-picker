...
import { MONTH_NAMES_EN, MONTH_NAMES_NP, DAY_NAMES_SHORT_EN, DAY_NAMES_SHORT_NP, toNepaliNumeral, } from '../core/format';
/**
 * Renders the full picker popover in day-selection mode.
 */
export function renderGrid(viewYear, viewMonth, selected, today, opts) {
 ...
 */
export function renderMonthGrid(viewYear, selectedMonth, opts) {
    const np = opts.language === 'np';
    const monthNames = np ? MONTH_NAMES_NP : MONTH_NAMES_EN;
    const popover = _makePopover('Month picker');
    popover.appendChild(_buildHeader(np ? toNepaliNumeral(viewYear) : String(viewYear), 'view-years', true, np));
    const grid = document.createElement('div');
 ...
 */
export function renderYearGrid(selectedYear, opts) {
    const np = opts.language === 'np';
    const rangeLabel = np
...
export function getCellDate(cell) {
